...
function useScreeningSimulation(isActive) {
  ...
  useEffect(() => {
    ...
    const t0 = setTimeout(
      () => setFacialMarkers(FACIAL_MARKER_STATES[1]),
      1500,
    );
...
export default function Screening() {
  ...
  const {
    ...
    faceDetected,
    finishSession,
  } = useLiveScreening(isActive, videoRef);

  function handleRestart() {
    window.location.reload(); // simplest way to fully reset the session for now
  }

  async function handleEndSession() {
    const result = await finishSession();
    if (result) {
      // Pass the real result to the Translate page via navigation state
      navigate("/translate", { state: { result } });
    }
  }
  return (
    <div
        ...
        <div style={{ marginBottom: "2rem" }}>
          ...
          >
            <div>
              ...
              <p
                className="body-sm text-ink-60"
                style={{ marginTop: "0.3rem", maxWidth: "520px" }}
              >
                Position yourself inside the frame. Sign Translator will follow
                your hands, face and movement.
              </p>
            </div>
        ...
        >
          ...
          >
            ...
            >
              ...
              <DetectionStatus handActive={handDetected} faceActive={faceDetected} />
            </div>
          </div>
            ...
            >
              ...
              >
                <p
                  className="label-sm"
                  style={{
                    color: isComplete ? "var(--sage)" : "var(--ink-60)",
                  }}
                >
                  {isComplete
                ...
                {isComplete && (
                  <span
                    className="label-mono"
                    style={{ color: "var(--clay)", fontSize: "0.65rem" }}
                  >
                    91%
                  </span>
                )}
